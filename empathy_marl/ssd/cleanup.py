"""Clean Up (Hughes et al. 2018) on the SSD grid world: apples, a polluted river and a cleaning beam.

Vendored from https://github.com/011235813/sequential_social_dilemma_games (MIT licence, see LICENSE
and NOTICE in this directory).  Changes: local imports, instance RNG (``self.rng``), the planner
variant is dropped.  Dynamics are unchanged (apple / waste spawning, beam behaviour, rewards).
"""
import numpy as np

from empathy_marl.ssd.agent import CleanupAgent
from empathy_marl.ssd.map_env import ACTIONS, MapEnv
from empathy_marl.ssd.maps import CLEANUP_MAP
...
class CleanupEnv(MapEnv):
    ...
    def __init__(self, ascii_map=CLEANUP_MAP, num_agents=1, render=False,
        ...
        for row in range(self.base_map.shape[0]):
            for col in range(self.base_map.shape[1]):
                # (upstream also appended the 'P' cells here a second time; MapEnv already collected them)
                if self.base_map[row, col] == 'B':
    ...
    def spawn_apples_and_waste(self):
        ...
        if not np.isclose(self.current_waste_spawn_prob, 0):
            self.rng.shuffle(self.waste_points)
