"""Base grid-world class of the Sequential Social Dilemma games (movement, beams, spawning).

Vendored from https://github.com/011235813/sequential_social_dilemma_games (Jiachen Yang's fork of
https://github.com/eugenevinitsky/sequential_social_dilemma_games, MIT licence, see LICENSE in this
directory).  Changes for this repository (see NOTICE): no Ray / matplotlib dependency, an instance
random generator (``self.rng``, seeded through :meth:`seed`) instead of the global ``numpy.random`` /
``random`` state, observations returned as *character* grids (the PettingZoo wrapper turns them into
planes or colours), ``render`` returns an RGB array.
"""
import numpy as np
...
class MapEnv:
    ...
    def __init__(self, ascii_map, num_agents=1, render=True, color_map=None,
        ...
        self.rng = np.random.default_rng()  # vendoring change: instance RNG (seed with .seed())
        ...
        self._base_spawn_points = [list(p) for p in self.spawn_points]  # map order, for seed()
        self.setup_agents()

    def seed(self, seed=None):
        """Re-seed the instance random generator (vendoring change).

        ``spawn_point`` shuffles ``self.spawn_points`` in place, so the list is also put back into map order here;
        otherwise the (unseeded) shuffles of earlier episodes would leak into a seeded one.
        """
        self.rng = np.random.default_rng(seed)
        self.spawn_points = [list(p) for p in self._base_spawn_points]
    ...
    def step(self, actions):
        ...
        for agent in self.agents.values():
            ...
            # vendoring change: character view (the wrapper renders planes / colours)
            observations[agent.agent_id] = self.rotate_view(agent.orientation, agent.get_state())
    ...
    def render(self):
        """RGB array (H, W, 3) of the whole map with agents and beams (vendoring change: no matplotlib)."""
        return self.map_to_colors(self.get_map_with_agents()).astype(np.uint8)
    ...
    def spawn_point(self):
        ...
        if self.shuffle_spawn:
            self.rng.shuffle(self.spawn_points)
    ...
    def spawn_rotation(self):
        ...
        if self.random_orientation:
            rand_int = int(self.rng.integers(len(ORIENTATIONS.keys())))
