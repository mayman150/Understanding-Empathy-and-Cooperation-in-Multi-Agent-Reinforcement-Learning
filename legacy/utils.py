...
class _MeltingPotPettingZooEnv(pettingzoo_utils.ParallelEnv):
  ...
  def __init__(self, env_config, max_cycles):
    ...
    self._env = substrate.build(
        'commons_harvest__open', roles=self.env_config.default_player_roles)
    self._num_players = len(self._env.observation_spec())
    self.possible_agents = [
        PLAYER_STR_FORMAT.format(index=index)
        for index in range(self._num_players)
    ]
    observation_space = utils_gym.remove_world_observations_from_space(
        utils_gym.spec_to_space(self._env.observation_spec()[0]))
    self.observation_space = functools.lru_cache(
        maxsize=None)(lambda agent_id: observation_space)
    action_space = utils_gym.spec_to_space(self._env.action_spec()[0])
    self.action_space = functools.lru_cache(maxsize=None)(
        lambda agent_id: action_space)
    self.state_space = utils_gym.spec_to_space(
  ...
  def reset(self, seed=None, options=None):
    ...
    self.num_cycles = 0
    return utils_gym.timestep_to_observations(timestep), {}
  ...
  def step(self, action):
    ...
    observations = utils_gym.timestep_to_observations(timestep)
