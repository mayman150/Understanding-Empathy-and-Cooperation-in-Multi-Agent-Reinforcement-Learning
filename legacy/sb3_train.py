...
class CustomCNN(torch_layers.BaseFeaturesExtractor):
  ...
  def __init__(
    ...
    self.conv = nn.Sequential(
        ...
        nn.ReLU(),  # 18 * 21 * 21
        nn.Conv2d(
            num_frames * 6, num_frames * 6, kernel_size=5, stride=2, padding=0),
        nn.ReLU(),  # 36 * 9 * 9
        nn.Conv2d(
            num_frames * 6, num_frames * 6, kernel_size=3, stride=1, padding=0),
        nn.ReLU(),  # 36 * 7 * 7
    ...
    flat_out =   num_frames * 6 * 7 * 7
    self.linear = nn.Sequential(nn.Linear(in_features=flat_out, out_features=fcnet_hiddens[0]), 
                             nn.ReLU(),
...
def main():
  ...
  # VecTransposeImage
  num_frames = 2 # num_frames = 4
  # output layer of cnn extractor AND shared layer for policy and value
  # functions
  features_dim = 16 # features_dim = 128
  fcnet_hiddens = [32, 16] # fcnet_hiddens = [1024, 128]  # Two hidden layers for cnn extractor
