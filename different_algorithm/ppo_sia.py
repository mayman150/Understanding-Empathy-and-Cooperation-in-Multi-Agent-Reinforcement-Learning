...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"ppo_sia_alpha_{args.alpha}"
    if args.track:
        import wandb
    ...
    for update in range(1, num_updates + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                #For aversion
                _, _, _, next_values, aversion_values = agents.get_actions_and_values(
                    # (B, 16, 88, 88, 3); (B, 16)
                    b_next_obs[mb_inds],
                    b_actions.long()[mb_inds],
                    aversion=1
    ...
    with open(f'results/models/ppo_sia_alpha_{str(args.alpha)}.pickle', 'wb') as handle:
