...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"ppo_svo_alpha_{args.alpha}"
    if args.track:
        import wandb
    ...
    for update in range(1, num_updates + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_advantages = b_advantages[mb_inds] + args.alpha * (math.sin(args.phi) * aversion_values + math.cos(args.phi) * next_values.reshape(-1, num_agents))
                if args.norm_adv:
    ...
    with open(f'results/models/ppo_svo_alpha_{str(args.alpha)}.pickle', 'wb') as handle:
