...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                aversion_values_alpha = args.alpha * torch.mean(torch.maximum((aversion_values.permute(2,1,0) - newvalue).permute(2,1,0), torch.zeros(aversion_values.shape).to(device)), dim=2).reshape(-1, num_agents) #2x7 
                aversion_values_beta = args.beta * torch.mean(torch.maximum((newvalue - aversion_values.permute(2,1,0)).permute(2,1,0), torch.zeros(aversion_values.shape).to(device)), dim=2).reshape(-1, num_agents)
                # 
                ...
                mb_advantages = b_advantages[mb_inds] - aversion_values_alpha - aversion_values_beta
                if args.norm_adv:
    ...
    with open(f'results/models/ppo_ei_alpha_{str(args.alpha)}.pickle', 'wb') as handle:
